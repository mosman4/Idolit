...
class  FooterCellViewModel: ObservableObject {
        ...
        func like() {
            ...
            if Auth.auth().currentUser!.uid != post.ownerId {
                ...
                let activityObject = Activity(activityId: activityId, type: "like", username: Auth.auth().currentUser!.displayName!, userId: Auth.auth().currentUser!.uid, userAvatar: Auth.auth().currentUser!.photoURL!.absoluteString, postId: post.postId,videodUrl:post.videodUrl!, mediaUrl: post.mediaUrl, comment: "", date: Date().timeIntervalSince1970)
